package org.scorelang.lexer;

public class ScoreKeywords {
	
	public static final String KW_TRUE		= "true";
	public static final String KW_FALSE		= "false";
	public static final String KW_IF		= "if";
	public static final String KW_ELSE		= "else";
	public static final String KW_FOR		= "for";
	public static final String KW_FOREACH	= "foreach";
	public static final String KW_WHILE		= "while";
	public static final String KW_BREAK		= "break";
	public static final String KW_CONTINUE	= "continue";
	public static final String KW_PRINT		= "print";
	public static final String KW_PRINTLN	= "println";
	public static final String KW_ERROR		= "error";
	public static final String KW_ERRORLN	= "errorln";
	public static final String KW_TYPEOF	= "typeof";
	public static final String KW_SLEEP		= "sleep";
	
}